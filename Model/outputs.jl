...
# --------------------------------------------------------------------------
# Build one summary record per scenario.
# The output tuple fields define the schema for base_results.csv and
# sensitivity_results.csv, so this function is the canonical KPI ledger.
# --------------------------------------------------------------------------
function build_summary_row(bundle, params::ModelParams; scenario_id::Int, scenario_name::String)
    ...
    # Pull all hourly primal decisions once for consistent KPI calculations.
    ...
    # Demand and non-served energy (NSE) diagnostics.
    ...
    # Renewable curtailment diagnostics.
    ...
    # Security diagnostics reconstructed from model equations.
    reserve_headroom = [
        (cap_oil - p_oil[t]) +
        (cap_gas - p_gas[t]) +
    ...
    # Objective breakdown components, read from JuMP expressions.
    ...
    # Weighted annual generation and storage throughput totals.
    ...
    # Return a flat named tuple to keep CSV output deterministic.
    return (
...
# --------------------------------------------------------------------------
# Build hour-by-hour dispatch export for the solved scenario.
# This table is used for plotting and diagnostic reconstruction.
# --------------------------------------------------------------------------
function build_dispatch_table(input_df::DataFrame, bundle)
    ...
    # Aggregate charge/discharge/SOC across battery duration classes.
    ...
    for h in H
        # Preserve per-duration trajectories to inspect duration-specific behavior.
...
# CSV writer helper for scenario summary outputs.
...
# CSV writer helper for hourly dispatch outputs.
function write_dispatch_csv(path::String, dispatch_df::DataFrame)
    CSV.write(path, dispatch_df)
    return dispatch_df
end

# Human-readable base-case console report used in quick checks.
...
# One-line sensitivity progress output for long scenario sweeps.
